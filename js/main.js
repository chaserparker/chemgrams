...
function chemspell(Word) {
    ...
    for (var i = 0; i < word_vector.length; i++) {
        word_vector[i] = word_vector[i][0].toUpperCase() + word_vector[i].substring(1);
    }
    return word_vector;
}
        ...
        if(rawFile.readyState === 4)
        {
            if(rawFile.status === 200 || rawFile.status == 0)
            {
                ...
                for (var i = 0; i < perfects.length; i++) {
                    if (perfects[i].includes(symbol)) {
                        var loc = perfects[i].search(symbol);
                        if (symbol.length === 1 && perfects[i][loc + 1].toString() === perfects[i][loc + 1].toString().toUpperCase()) { // prevents 1-letter symbols from appearing where they shouldn't (e.g. "HeAt" contains "He" NOT "H")
